...
<head>
    ...
    <style>
        ...
        .tabla-empleados td.td-acciones {
            white-space: nowrap;
        }
        .tabla-scroll {
            overflow-x: auto;
...
<div class="pagina">
    ...
    <div class="encabezado">
        <div>
            <div class="encabezado-titulo"><i class="bi bi-person-badge me-2"></i>Panel de Empleados</div>
            <div class="encabezado-sub">Lista de Empleados</div>
        </div>
    ...
    <?php if (empty($resultado) || count($resultado) == 0): ?>
        <div class="vacio" style="text-align: center; padding: 4rem 2rem; background: #fff; border-radius: 12px; border: 1px dashed #cbd5e1;">
            ...
            <p style="color: #64748b;">Aún no hay empleados registrados en el sistema.</p>
            <a href="../controller/empleadoRegistroCo.php" class="btn-nuevo mt-2"><i class="bi bi-plus-lg me-1"></i>Agregar el primero</a>
        </div>
    <?php else: ?>
        <div class="contador">Mostrando <?= count($resultado) ?> empleado<?= count($resultado) != 1 ? 's' : '' ?></div>
        ...
        <div class="tabla-scroll">
            <table class="tabla-empleados">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="text-align:center; color:var(--txt-claro); font-weight:700;"><?= $i + 1 ?></td>
                        <td><span class="etiqueta"><?= htmlspecialchars($reg['cargo']) ?></span></td>
                        <td><?= htmlspecialchars($reg['ci']) ?></td>
                        <td style="font-weight:600; color: #0f172a;"><?= htmlspecialchars($reg['nombre'] . ' ' . $reg['paterno'] . ' ' . $reg['materno']) ?></td>
                        <td><?= htmlspecialchars($reg['direccion']) ?></td>
                        <td><?= htmlspecialchars($reg['telefono']) ?></td>
                        <td><?= htmlspecialchars($reg['fechanacimiento']) ?></td>
                        <td><?= htmlspecialchars($reg['genero']) ?></td>
                        <td><?= htmlspecialchars($reg['intereses']) ?></td>
                        <td class="td-acciones">
                            <a href="../controller/empleadoModifica.php?cod=<?= $reg['id_empleado'] ?>" class="btn-editar">
                                <i class="bi bi-pencil-square me-1"></i>Modificar
                            </a>
                            <a href="../controller/empleadoEliminar.php?cod=<?= $reg['id_empleado'] ?>" class="btn-eliminar"
                                onclick="return confirm('¿Seguro que deseas eliminar al empleado <?= htmlspecialchars(addslashes($reg['nombre'] . ' ' . $reg['paterno'])) ?>?')">
