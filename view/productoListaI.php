<?php
/*
 * Panel de Productos Inactivos
 * Estilo similar a clientes inactivos
 */
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Panel - Productos Inactivos</title>
    ...
    <style>
        ...
        .badge-precio { background: #fce7f3; color: #be185d; font-weight: 700; }
        .badge-stock { background: #d1fae5; color: #047857; font-weight: 700; }

        .btn-nuevo, .btn-buscar, .btn-salir, .btn-activar, .btn-eliminar {
            display: inline-flex;
            align-items: center;
            padding: 0.5rem 1rem;
            border-radius: 8px;
        ...
        .btn-activar { background: #f0fdf4; color: #16a34a; border: 1px solid #bbf7d0; margin-right: .5rem; }
        .btn-activar:hover { background: #dcfce7; color: #15803d; }
...
<div class="pagina">
    ...
    <div class="encabezado">
        <div>
            <div class="encabezado-titulo"><i class="bi bi-box-seam me-2"></i>Panel de Productos Inactivos</div>
            <div class="encabezado-sub">Lista de Productos inactivos</div>
        </div>
        <div style="display:flex; gap:.6rem; align-items:center; flex-wrap: wrap;">
            <a href="../controller/productoLista.php" class="btn-buscar">
                <i class="bi bi-arrow-left me-1"></i>Activos
            </a>
    ...
    <?php if (empty($resultado) || count($resultado) == 0): ?>
        <div class="vacio" style="text-align: center; padding: 4rem 2rem; background: #fff; border-radius: 12px; border: 1px dashed #cbd5e1;">
            ...
            <h4 style="color: #334155; font-weight: 700;">No hay productos inactivos</h4>
            <p style="color: #64748b;">Todos los productos están activos o no existen registros inactivos.</p>
        </div>
    <?php else: ?>
        <div class="contador">Mostrando <?= count($resultado) ?> producto<?= count($resultado) != 1 ? 's' : '' ?> inactivo<?= count($resultado) != 1 ? 's' : '' ?></div>
        ...
        <div class="tabla-scroll">
            <table class="tabla-productos">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="text-align:center; color:#475569; font-weight:700;">#<?= htmlspecialchars($reg['id_producto'], ENT_QUOTES) ?></td>
                        <td><?= htmlspecialchars($reg['proveedor'] ?? 'Sin proveedor', ENT_QUOTES) ?></td>
                        <td style="font-weight:600; color: #0f172a; text-decoration: line-through; opacity: 0.8;"><?= htmlspecialchars($reg['nombreproducto'], ENT_QUOTES) ?></td>
                        <td><?= htmlspecialchars($reg['tipo'], ENT_QUOTES) ?></td>
                        <td><span class="badge-precio">Bs. <?= number_format($reg['precio'], 2, ',', '.') ?></span></td>
                        <td><span class="badge-stock"><?= htmlspecialchars($reg['stock'], ENT_QUOTES) ?></span></td>
                        <td>
                        ...
                        <td class="td-acciones">
                            <a href="../controller/productoActivar.php?cod=<?= htmlspecialchars($reg['id_producto'], ENT_QUOTES) ?>" class="btn-activar"
                                onclick="return confirm('¿Seguro que deseas activar el producto: <?= htmlspecialchars(addslashes($reg['nombreproducto'])) ?>?')">
                                <i class="bi bi-check-circle-fill me-1"></i>Activar
                            </a>
                            <a href="../controller/productoEliminar.php?cod=<?= htmlspecialchars($reg['id_producto'], ENT_QUOTES) ?>" class="btn-eliminar"
                                onclick="return confirm('¿Eliminar PERMANENTEMENTE el producto: <?= htmlspecialchars(addslashes($reg['nombreproducto'])) ?>?')">
