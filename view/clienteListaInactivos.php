...
<head>
    ...
    <title>Panel - Clientes Inactivos</title>
    ...
    <style>
        ...
        .etiqueta { background: #f1f5f9; color: #475569; padding: 0.2rem 0.6rem; border-radius: 6px; font-size: 0.8rem; font-weight: 600; border: 1px solid #e2e8f0; }
        
        /* Botones unificados */
        .btn-buscar, .btn-salir, .btn-eliminar, .btn-activar {
        ...
        /* Nuevo botón para Activar */
        .btn-activar { background: #f0fdf4; color: #16a34a; border: 1px solid #bbf7d0; margin-right: 0.5rem; }
...
<div class="pagina">
    ...
    <div class="encabezado">
        <div>
            <div class="encabezado-titulo"><i class="bi bi-person-slash me-2"></i>Panel de Clientes Inactivos</div>
            <div class="encabezado-sub">Lista de Clientes Inactivos</div>
        </div>
        <div style="display:flex; gap:.6rem; align-items:center; flex-wrap: wrap;">
            <a href="../controller/clienteBusquedaInac.php" class="btn-buscar">
    ...
    <?php if (empty($resultado) || count($resultado) == 0): ?>
        <div class="vacio" style="text-align: center; padding: 4rem 2rem; background: #fff; border-radius: 12px; border: 1px dashed #cbd5e1;">
            ...
            <h4 style="color: #334155; font-weight: 700;">No existen registros inactivos</h4>
            <p style="color: #64748b;">Todos los clientes se encuentran activos o no hay registros en la base de datos.</p>
        </div>
    <?php else: ?>
        <div class="contador">Mostrando <?= count($resultado) ?> cliente<?= count($resultado) != 1 ? 's' : '' ?> inactivo<?= count($resultado) != 1 ? 's' : '' ?></div>
        ...
        <div class="tabla-scroll">
            <table class="tabla-clientes">
                ...
                <tbody>
                    ...
                    <tr>
                        <td style="text-align:center; color:var(--txt-claro); font-weight:700;">#<?= $reg['id_cliente'] ?></td>
                        <td><span class="etiqueta"><?= htmlspecialchars($reg['nit_ci']) ?></span></td>
                        <td style="font-weight:600; color: #0f172a; text-decoration: line-through; opacity: 0.7;">
                            <?= htmlspecialchars($reg['razonsocial']) ?>
                        </td>
                        <td class="td-acciones">
                            <a href="../controller/clienteActivar.php?cod=<?= $reg['id_cliente'] ?>" class="btn-activar"
                                onclick="return confirm('¿Seguro que deseas reactivar al cliente: <?= htmlspecialchars(addslashes($reg['razonsocial'])) ?>?')">
                                <i class="bi bi-check-circle-fill me-1"></i>Activar
                            </a>
                            <a href="../controller/clienteEliminar.php?cod=<?= $reg['id_cliente'] ?>" class="btn-eliminar"
                                onclick="return confirm('¿Seguro que deseas ELIMINAR PERMANENTEMENTE al cliente: <?= htmlspecialchars(addslashes($reg['razonsocial'])) ?>?')">
